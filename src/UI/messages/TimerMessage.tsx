...
const TimerMessage = (props: ActionMessageProps) => {
    ...
    return (
        <animated.div style={{ ...springs }} className={`mt-1 mb-1 p-[8px] flex flex-col items-center w-full ${props.isLast && 'pb-[100px]'}`}>
            ...
            >
                ...
                {Number(props.text) === -1 ? 'removed self-destruct timer' :
                <>set self-destruct timer to<a className='font-bold'> {getStringFromMs(props.text)}</a></> }
            </section>
        </animated.div>
    );
};
